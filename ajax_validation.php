...
if ($_REQUEST["Type"] == "AddToWatchListSEAERCH") {
    function validate($vehicleid, $userid)
    {
        if ($userid == "") {
            ...
            $SE = "SELECT vid, userid FROM mutual_watchinglist WHERE vid='" . $vehicleid . "' AND userid='" . $userid . "'";
            $SERs = mysql_query($SE);
            $TOT = mysql_num_rows($SERs);
            if ($TOT > 0) {
                return "Already added";
            } else {
                $InsertNewsletterQry = "INSERT INTO mutual_watchinglist SET vid='" . $vehicleid . "', userid='" . $userid . "'";
    ...
    function validate($itemSKU)
    {
        ...
        if ($righttotcartitem > 0) {
            ...
            for ($CL = 0; $CL < count($SESSION_itemid); $CL++) {
                $ProductQry = "select product_title,id,user_defined_sku_id,item_price  from mutual_searplusitems where id='" . $SESSION_itemid[$CL] . "'";
