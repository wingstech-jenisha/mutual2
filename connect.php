...
{
$urlCCCC = "http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'];
//echo "<script>window.location.href='".$urlCCCC."';</ script>";
}*/
...
{
    global $SITE_URL;
    $sku_id = stripslashes(getname("mutual_searplusitems", $PId, 'user_defined_sku_id'));
    $title = stripslashes(getname("mutual_searplusitems", $PId, 'product_title'));
    $prodname1 = strtolower(getModifiedUrlNamechangeJET($title));
    $prodname1stag1 = str_replace("---", "-", $prodname1);
    $productname = str_replace("--", "-", $prodname1stag1);
    $brand = stripslashes(getname("mutual_searplusitems", $PId, 'brand'));
    /*if(trim($brand)!='')
	{
		$brand=strtolower(getModifiedUrlNamechangeJET($brand));
		$brand=str_replace("--","-",$brand);
...
function Get_CartProductUrl($PId)
{
    global $SITE_URL;
    $sku_id = $PId;
    $title = stripslashes(GetName1("mutual_searplusitems", "product_title", "user_defined_sku_id", $sku_id));
    $prodname1 = strtolower(getModifiedUrlNamechangeJET($title));
    $prodname1stag1 = str_replace("---", "-", $prodname1);
    $productname = str_replace("--", "-", $prodname1stag1);
    $brand = stripslashes(GetName1("mutual_searplusitems", "brand", "user_defined_sku_id", $sku_id));
    /*if(trim($brand)!='')
	{
		$brand=strtolower(getModifiedUrlNamechangeJET($brand));
		$brand=str_replace("--","-",$brand);
...
{
    global $SITE_URL;
    $title = stripslashes(GetName1("mutual_category", "jet_node_name", 'jet_node_id', $PId));
    $prodname1 = strtolower(getModifiedUrlNamechangeJET($title));
    $prodname1stag1 = str_replace("---", "-", $prodname1);
    $productname = str_replace("--", "-", $prodname1stag1);
...
{
    global $SITE_URL;
    $title = stripslashes(GetName1("mutual_blog", "title", 'id', $PId));
    $prodname1 = strtolower(getModifiedUrlNamechangeJET($title));
    $prodname1stag1 = str_replace("---", "-", $prodname1);
    $productname = str_replace("--", "-", $prodname1stag1);
...
function GetABulletsNames($Valll)
{
    ...
    if ($Valll != "" && $Valll != ",") {
        ...
        if (mysql_affected_rows() > 0) {
            while ($RsOW = mysql_fetch_object($SEssLRs)) {
                $tagss .= stripslashes($RsOW->name) . ", ";
            }
        }
    }
    if ($tagss != "") {
